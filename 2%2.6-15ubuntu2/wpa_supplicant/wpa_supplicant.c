							  ...
							  enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection)
	{
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CONNECTED "- Connection to " MACSTR " completed [id=%d id_str=%s]",
				MAC2STR(wpa_s->bssid),
				ssid ? ssid->id : -1,
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	reconf_ctrl = !!conf->ctrl_interface != !!wpa_s->conf->ctrl_interface || (conf->ctrl_interface && wpa_s->conf->ctrl_interface &&
	...
	else
	{
		...
		if (wpa_supplicant_suites_from_ai(wpa_s, ssid, &ie) < 0)
		{
			...
			ie.mgmt_group_cipher =
				ssid->ieee80211w != NO_MGMT_FRAME_PROTECTION ? WPA_CIPHER_AES_128_CMAC : 0;
#endif /* CONFIG_IEEE80211W */
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (bss)
	{
		...
		if (!wpas_driver_bss_selection(wpa_s) || ssid->bssid_set ||
			wpa_s->key_mgmt == WPA_KEY_MGMT_WPS)
		{
			wpa_printf(MSG_DEBUG, "Limit connection to BSSID " MACSTR " freq=%u MHz based on scan results "
								  "(bssid_set=%d wps=%d)",
								   ...
								   int reason_code)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Request to deauthenticate - bssid=" MACSTR " pending_bssid=" MACSTR " reason=%d state=%s",
				   ...
				   void *ctx)
{
	...
	else if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_SIMULTANEOUS) && radio->num_active_works < MAX_ACTIVE_WORKS)
	{
		wpa_dbg(wpa_s, MSG_DEBUG,
							  ...
							  struct wpa_ssid *ssid, int clear_failures)
{
	...
	{
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_REENABLED "id=%d ssid=\"%s\"",
				ssid->id, wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
	}
...
static struct
	wpa_bss_tmp_disallowed *
	wpas_get_disallowed_bss(struct wpa_supplicant *wpa_s,
							const u8 *bssid)
{
