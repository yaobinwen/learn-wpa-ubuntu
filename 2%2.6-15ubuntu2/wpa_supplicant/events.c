									...
									int only_first_ssid)
{
	...
	for (ssid = group; ssid; ssid = only_first_ssid ? NULL : ssid->pnext)
	{
		...
		if (!is_zero_ether_addr(ssid->go_p2p_dev_addr))
		{
			...
			if (p2p_parse_dev_addr_in_p2p_ie(p2p_ie, dev_addr) < 0 || os_memcmp(dev_addr, ssid->go_p2p_dev_addr,
																				ETH_ALEN) != 0)
			{
										  ...
										  int locally_generated)
{
	...
	if (!is_zero_ether_addr(bssid) ||
		wpa_s->wpa_state >= WPA_AUTHENTICATING)
	{
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "bssid=" MACSTR " reason=%d%s",
						  ...
						  union wpa_event_data *data)
{
	...
	case EVENT_ASSOC_REJECT:
		if (data->assoc_reject.bssid)
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_ASSOC_REJECT "bssid=" MACSTR " status_code=%u%s",
...
#endif /* CONFIG_AP */
#ifdef CONFIG_OFFCHANNEL
		wpa_dbg(wpa_s, MSG_DEBUG, "EVENT_TX_STATUS pending_dst=" MACSTR, MAC2STR(wpa_s->p2pdev->pending_action_dst));
		/*
	...
	case EVENT_CH_SWITCH:
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CHANNEL_SWITCH "freq=%d ht_enabled=%d ch_offset=%d ch_width=%s cf1=%d cf2=%d",
				data->ch_switch.freq,
